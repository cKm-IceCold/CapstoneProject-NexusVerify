...
function SearchBar() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.success) {
        alert(`${result.message}: ${result.data.currency} ${result.data.price.toLocaleString()}`);
      } else {
        alert(result.message || "No data found for this selection.");
      }
    } catch (error) {
  ...
  return (
    <div className="w-full max-w-md md:max-w-2xl relative z-40 mt-6 px-2 md:px-0">
      <div className="flex flex-col md:flex-row bg-white rounded-xl shadow-2xl p-2 md:p-0">
